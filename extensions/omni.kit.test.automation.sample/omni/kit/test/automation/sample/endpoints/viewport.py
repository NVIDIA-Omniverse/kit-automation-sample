...
from omni.kit.ui_test import WidgetRef, Vec2
...
LOG_RENDERER_CHANGE = "Changing the active renderer to"
LOG_RENDERER_SET = "Set viewport renderer as"
LOG_RENDERER_INVALID = "Invalid renderer received, valid options are"
...
async def zoom_viewport(request: ZoomViewportRequest):
    ...
    try:
        ...
        if "/OmniverseKit_Persp" in camera_path:
            xform = UsdHelper.getXform_from_current_stage(camera_path)
            UsdHelper.add_and_set_test_camera(stage, xform=xform)
            camera_path = UsdHelper.get_default_camera_path()
        if (stage, camera_path) not in camera_initial_state:
            camera_initial_state[(stage, camera_path)] = UsdHelper.getXform_from_current_stage(
                camera_path
            )
...
async def toggle_renderer(renderer: str):
    ...
    if renderer.lower() in renderers.keys():
        ...
        encoded_renderer = _encode_for_logging(renderer)
        logger.info("%s: %s", LOG_RENDERER_CHANGE, encoded_renderer)
        ...
        encoded_renderer = _encode_for_logging(renderer)
        logger.info("%s: %s", LOG_RENDERER_SET, encoded_renderer)
        return MessageResponse(message=f"Set viewport renderer as: {renderer}")
    else:
        logger.error("%s: %s", LOG_RENDERER_INVALID, list(renderers.keys()))
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail=f"{LOG_RENDERER_INVALID}: {list(renderers.keys())}"
        )
