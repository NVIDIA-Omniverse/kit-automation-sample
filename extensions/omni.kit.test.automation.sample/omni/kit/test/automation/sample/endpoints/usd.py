...
LOG_PRIM_COORDINATES = "Fetched coordinates of prim at path"
LOG_PRIM_SCREEN_ERROR = "Failed to get screen coordinate for prim"
...
async def prim_transform_properties(prim_path: str):
    ...
    try:
        ...
        logger.info(
            "%s \nTranslate: %s, Rotate: %s, Scale: %s, Pivot: %s",
            LOG_PRIM_COORDINATES,
            translate,
            rotate,
            scale,
            pivot
        ...
        encoded_path = _encode_for_logging(prim_path)
        logger.error("%s Error: %s", LOG_PRIM_COORDINATES, str(e))
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail=f"Prim invalid/Prim does not exist at path '{encoded_path}': {str(e)}"
        )
...
def get_prim_screen_coordinate(prim_path: str) -> Dict[str, Dict[str, float]]:
    ...
    try:
        ...
        logger.error("%s Error: %s", LOG_PRIM_SCREEN_ERROR, str(exc))
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Failed to get screen coordinate for prim: {str(exc)}"
        )
